...
def buscar_pokemon():
    ...
    if response.status_code == 200:  # Verifica si la solicitud fue exitosa (código de estado 200)
        ...
        with open(f"{nombre}.txt", "w") as archivo:  # Abre un archivo en modo escritura con el nombre del Pokémon y extensión .txt
            archivo.write(resultado)  # Escribe el resultado (información del Pokémon) en el archivo
        
        # Crear un diccionario con los datos del Pokémon y la URL de la imagen
        datos_pokemon = {
            "nombre": nombre,
            "numero": numero,
            "tipos": tipos,
        ...
        # Generar archivo JSON con los datos del Pokémon
        with open(f"{nombre}.json", "w") as archivo_json:  # Abre un archivo en modo escritura con el nombre del Pokémon y extensión .json
            json.dump(datos_pokemon, archivo_json)  # Escribe los datos del Pokémon en formato JSON en el archivo
        
        label_resultado.config(text="Archivo generado exitosamente.")  # Configura la etiqueta label_resultado para mostrar un mensaje de éxito
    else:
        ...
        label_resultado.config(text=resultado)  # Configura la etiqueta label_resultado para mostrar el mensaje de error
